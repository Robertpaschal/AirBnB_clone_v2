...
"""
    ...
    def test_do_create_with_params(self):
        """Tests create with parameters."""
        cases = [
            ("User email=test@example.com", {"email": "test@example.com"}),
            ("State name=\"New York\"", {"name": "New York"}),
            ("City state_id=1 name=\"New York City\"", {
                "state_id": "1", "name": "New York City"}),
            ("Amenity name=\"Wifi\"", {"name": "Wifi"}),
            ("Place city_id=1 user_id=2 name=\"Cozy Apartment\" \
              description=\"Beautiful view\" \
              number_rooms=2 "
             "number_bathrooms=1 max_guest=4 price_by_night=1000.50 \
                  latitude=40.7128 longitude=-74.0060 amenity_ids=[1, 2, 3]",
             {"city_id": "1", "user_id": "2", "name":
              "Cozy Apartment", "description": "Beautiful view",
              "number_rooms": 2, "number_bathrooms":
              1, "max_guest": 4, "price_by_night": 1000.50,
              "latitude": 40.7128, "longitude":
              -74.0060, "amenity_ids": [1, 2, 3]}),
        ]
        for command, expected_attributes in cases:
            with patch('sys.stdout', new=StringIO()) as f:
                HBNBCommand().onecmd(command)
            created_object_id = f.getvalue().strip()

            with open("file.json", 'r') as file:
                objects = json.load(file)

            created_object = objects[
                "{}.{}".format(command.split()[0], created_object_id)]
            created_object_attributes = {
                key.replace(
                    "_", " "): value for key, value in created_object.items()}

            self.assertDictEqual(
                expected_attributes, created_object_attributes)
